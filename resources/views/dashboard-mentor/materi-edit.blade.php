    ...
    <div class="bg-white shadow-md rounded-lg p-6 border border-gray-200">
        ...
        <form action="{{ route('materi.update', ['courseId' => $course->id, 'materiId' => $materi->id]) }}" method="POST" enctype="multipart/form-data" class="w-full">
            ...
            <div class="grid grid-cols-1 gap-2">
                ...
                <div 
                    x-data="{
                        materiDrive: @js(old('type') ? collect(old('type'))->map(function($type, $i) {
                            if ($type === 'drive') {
                                return [
                                    'title' => old('title')[$i] ?? '',
                        ...
                        })->filter()->values() : $materi->videos->map(function($drive) {
                            return [
                                'title' => $drive->title,
                        ...
                        materiYoutube: @js(old('type') ? collect(old('type'))->map(function($type, $i) {
                            if ($type === 'youtube') {
                                return [
                                    'title' => old('title')[$i] ?? '',
                                    'description' => old('description')[$i] ?? '',
                        ...
                        })->filter()->values() : $materi->youtube->map(function($yt) {
                            return [
                                'title' => $yt->title,
