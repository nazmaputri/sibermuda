...
<div class="max-w-2xl border border-gray-200 mx-auto bg-white p-6 rounded-lg shadow-lg">
    ...
    <form action="{{ url('/update-affiliate') }}" method="POST" enctype="multipart/form-data">
    @csrf
    @method('PUT')
        <div class="mb-4">
            ...
            @if($user->photo)
                <img src="{{ route('user.avatar', $user->id) }}" alt="Foto Profil" class="w-28 h-28 rounded-full mx-auto object-cover">
            @endif

            <input type="file" name="photo" id="photo" accept="image/*" class="p-1.5 text-sm mt-2 block w-full border border-gray-200 rounded-md shadow-sm text-gray-700">
            @error('photo')
        ...
        <div class="mb-4 relative">
            ...
            <div class="relative">
                <input type="password" name="password" id="password" id="new-password"
                    class="password-input p-1.5 pr-10 mt-1 block w-full text-sm text-gray-700 border-gray-200 border rounded-md shadow-sm focus:outline-none focus:ring-1 focus:ring-gray-400 focus:border-gray-400">
       ...
       <div class="mb-4 relative">
        ...
        <div class="flex justify-end space-x-4">
            <!-- Tombol Batal -->
            <a href="{{ route('welcome-affiliate') }}" class="bg-red-400 text-white font-medium py-2 px-6 rounded-lg hover:bg-red-300 text-sm">Batal</a>
            <!-- Tombol Simpan -->
