...
<div class="container mx-auto">
    <div class="bg-white shadow-md rounded-lg p-6 mb-6 border border-gray-200">
        <div class="flex flex-col md:flex-row items-center justify-between space-y-4 mb-4">
            <form action="{{ route('admin.data-affiliate.index') }}" method="GET" class="w-full md:max-w-xs">
                <label for="search" class="mb-2 text-sm font-medium text-gray-900 sr-only">Cari</label>
                    <div class="flex items-center border border-gray-300 rounded-lg bg-white">
                        ...
                        <input type="search" name="search" id="search"
                            ...
                            placeholder="Cari Nama dan Status affiliate" value="{{ request('search') }}" />
                    </div>
            </form>

            <a href="{{ route('admin.data-affiliate.create') }}"  class="ml-4 text-white px-4 py-2 font-semibold rounded-md shadow-blue-100 bg-blue-400 hover:bg-blue-300 focus:outline-none flex items-center">
                        ...
                        <tr class="bg-white hover:bg-gray-50 user-row" data-role="{{ $user->role }}">
                            ...
                            <td class="py-3 px-6 text-center border-b border-r border-gray-200">
                                <div class="flex items-center justify-center space-x-8">
                                    <!-- Toggle Aktif/Nonaktifkan affiliate -->
                                    ...
                                    <!-- Tombol Lihat Detail -->
                                    {{-- <a href="{{ route('detaildata-affiliate', ['id' => $user->id]) }}" class="text-white bg-sky-300 p-1 rounded-md hover:bg-sky-200" title="Lihat">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 0 1 0-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178Z" />
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z" />
                                        </svg>
                                    </a>
                                    <form action="{{ route('dataaffiliate-admin.delete', $user->id) }}" method="POST" class="inline">
                                        @csrf
                                        @method('DELETE')
