                ...
                <table class="min-w-full text-sm border-separate border-spacing-0">
                    <thead>
                        <tr class="bg-gray-100 text-gray-600 text-sm">
                            ...
                            <th class="px-4 py-2 text-center border-b border-t border-gray-200">Harga Kursus</th>
                    ...
                    <tbody class="text-gray-600 text-sm">
                        @forelse($purchasedCourses as $index => $purchase)
                            <tr class="bg-white hover:bg-gray-50 border-b text-sm">
                                ...
                                <td class="px-4 py-2 border-b border-gray-200 capitalize text-center">
                                    @if(optional($purchase)->harga_course == 0)
                                        Payment Manual
                                    @else
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="text-center py-2 px-2 text-gray-600 border-l border-b border-r">Belum ada kursus yang dibeli</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
...
<script>
function confirmUpdate(button, route) {
    const userName = button.dataset.nama;
    const amount = button.dataset.amount;

    Swal.fire({
        title: 'Konfirmasi Perubahan Status',
        html: `<div class="text-sm">
                  <p>Apakah Anda yakin ingin mengubah status pembayaran?</p>
                  <br>
                  <p><strong>Nama :</strong> ${userName}</p>
                  <p><strong>Jumlah Dibayar :</strong> Rp ${amount}</p>
               </div>`,
        icon: 'warning',
        customClass: {
            popup: 'text-sm',
