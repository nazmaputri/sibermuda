...
<div class="container mx-auto bg-white rounded-lg p-5 border border-gray-200">
    ...
    <div class="mb-6 flex items-center justify-between">
        <h2 class="text-2xl font-bold text-gray-700">Edit Bootcamp</h2>
    ...
    <form action="{{ route('admin.bootcamp.update', $bootcamp->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="title" id="title" value="{{ old('title', $bootcamp->title) }}"
        ...
        <div class="mb-4">
            ...
            <textarea name="description" id="description" rows="4"
                ...
                placeholder="Deskripsi lengkap bootcamp..." required>{{ old('description', $bootcamp->description) }}</textarea>
            @error('description')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            <div>
                ...
                <input type="text" name="duration" id="duration" value="{{ old('duration', $bootcamp->duration) }}"
            ...
            <div>
                ...
                <select name="level" id="level"
                    ...
                    <option value="Pemula" {{ old('level', $bootcamp->level) == 'Pemula' ? 'selected' : '' }}>Pemula</option>
                    <option value="Pemula - Menengah" {{ old('level', $bootcamp->level) == 'Pemula - Menengah' ? 'selected' : '' }}>Pemula - Menengah</option>
                    <option value="Menengah" {{ old('level', $bootcamp->level) == 'Menengah' ? 'selected' : '' }}>Menengah</option>
                    <option value="Menengah - Lanjutan" {{ old('level', $bootcamp->level) == 'Menengah - Lanjutan' ? 'selected' : '' }}>Menengah - Lanjutan</option>
                    <option value="Lanjutan" {{ old('level', $bootcamp->level) == 'Lanjutan' ? 'selected' : '' }}>Lanjutan</option>
                </select>
                @error('level')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
        ...
        <div class="mb-4">
            ...
            <input type="text" name="schedule" id="schedule" value="{{ old('schedule', $bootcamp->schedule) }}"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            <div>
                ...
                <input type="text" name="price" id="price" value="{{ old('price', $bootcamp->price) }}"
            ...
            <div>
                ...
                <input type="text" name="discount_price" id="discount_price" value="{{ old('discount_price', $bootcamp->discount_price) }}"
        ...
        <div class="mb-4">
            ...
            <div class="mb-3">
                ...
                <img src="{{ asset('storage/' . $bootcamp->image) }}" alt="{{ $bootcamp->title }}" class="w-32 h-32 object-cover rounded-lg border border-gray-300">
            </div>
            @endif
        ...
        <div class="mb-4">
            ...
            <div id="featuresContainer">
                @if(old('features'))
                    ...
                    @endforeach
                @elseif($bootcamp->features && count($bootcamp->features) > 0)
        ...
        <div class="mb-4">
            ...
            <div id="syllabusContainer">
                @if(old('syllabus'))
                    @foreach(old('syllabus') as $item)
                    <div class="flex items-center space-x-2 mb-2">
                        <input type="text" name="syllabus[]" value="{{ $item }}"
                            class="flex-1 px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-sky-400"
                    ...
                    @endforeach
                @elseif($bootcamp->syllabus && count($bootcamp->syllabus) > 0)
                    @foreach($bootcamp->syllabus as $item)
                    <div class="flex items-center space-x-2 mb-2">
                        <input type="text" name="syllabus[]" value="{{ $item }}"
                            class="flex-1 px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-sky-400"
                            placeholder="Contoh: HTML & CSS Fundamentals">
        ...
        <div class="mb-6">
            <label class="flex items-center space-x-2 cursor-pointer">
                <input type="checkbox" name="is_active" value="1" {{ old('is_active', $bootcamp->is_active) ? 'checked' : '' }}
